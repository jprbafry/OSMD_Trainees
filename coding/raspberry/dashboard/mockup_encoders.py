...
def update_motor_encoders(sd: SensorData, lock: threading.Lock, step=1, period_ms=250):
    direction = [1]*4
    while True:
        with lock:
            for i in range(4):
...
def update_home_switches(sd: SensorData, lock: threading.Lock, period_ms=100):
    while True:
        with lock:
            for i in range(4):
                sd.home_switches[i] = int(sd.motor_encoders[i] == 0)
        time.sleep(period_ms / 1000)

def update_potentiometers(sd: SensorData, lock: threading.Lock, period_ms=250):
    while True:
        with lock:
...
def update_ref_diode(sd: SensorData, lock: threading.Lock, period_ms=250):
    while True:
        with lock:
            sd.ref_diode = 650 + random.randint(-20,20)
        time.sleep(period_ms / 1000)

def update_temperature(sd: SensorData, lock: threading.Lock, period_ms=1000):
...
def update_imu(sd: SensorData, lock: threading.Lock, period_ms=100):
    start_time = time.time()
...
def parse_args():
    ...
    parser.add_argument("--port", "-p", default="/dev/ttyACM0",
                        help="Serial port to use when not simulating (e.g. /dev/ttyACM0)")
